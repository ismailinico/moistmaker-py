...
| padding         | float **or** [int, int] | 0 to 1 **or** [x, y], where x and y are whole numbers. | 0.6                          | Either an int list, from which the [0] defines the horizontal and [1] the vertical padding in pixels, or a percentage of the watermark's pixel height, similar to the CSS unit vh, which will then be used as padding on both axes.                   |
...
| threshold       | int                     | 0 to 255                                               | 150                          | Defines the threshold value of when the algorithm defines an area as bright or dark. **Only change this value if there is an issue with brightness detection.**                                                  |
